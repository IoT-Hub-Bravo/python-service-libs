...
class KafkaConsumer:
    ...
    def _handle_payload(self, payload: Any, topic: str) -> bool:
        """
        ...
        try:
            ...
            kafka_messages_total.labels(topic=topic, status='success').inc()
            return True
        except Exception:
            logger.exception('Failed to handle Kafka message payload.')
            kafka_errors_total.labels(topic=topic, error_type='handler_error').inc()
            return False
        finally:
            kafka_latency_seconds.labels(topic=topic).observe(
    ...
    def _handle_and_commit(self, payload: Any, message: Message) -> None:
        ...
        """
        handled_successfully = self._handle_payload(payload, message.topic())
    ...
    def _get_message_payload(self, message: Message) -> Optional[Any]:
        ...
        if payload is None:
            kafka_errors_total.labels(topic=message.topic(), error_type='decode_error').inc()
        return payload
